...
import com.baipengx.coin.service.impl.WechatMessageServiceImpl;
...
public class SendTemplateMessageUtil extends BaseLogger {
	...
	public  String sendMsg(BaseTemplateMessage msg){
		...
		try {
			...
			throw new SendTemplateMsgException(WechatMessageServiceImpl.FAIL_SEND_STATUS);
		}
		
		if(response.getErrcode() == 0){
			return response.getMsgid();
		}else if (response.getErrcode() == 43004){
			throw new SendTemplateMsgException(WechatMessageServiceImpl.UNSUB_SEND_STATUS);
		}
		return null;
	}
